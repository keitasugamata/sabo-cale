...
import { cloudGetAllTags, cloudSaveTag, cloudSaveTags, cloudDeleteTag } from '../cloudDb';
...
export function useTags(user) {
  ...
  useEffect(() => {
    setLoaded(false);
    const loader = isCloud ? cloudGetAllTags(user.id) : getAllTags();
    loader.then(async (data) => {
      const whereExists = data.some((t) => t.category === 'where');
      if (data.length === 0) {
        if (isCloud) await cloudSaveTags(DEFAULT_TAGS, user.id);
        else await Promise.all(DEFAULT_TAGS.map(saveTag));
        setTags(DEFAULT_TAGS);
      } else if (!whereExists) {
        const whereTags = DEFAULT_TAGS.filter((t) => t.category === 'where');
        if (isCloud) await cloudSaveTags(whereTags, user.id);
        else await Promise.all(whereTags.map(saveTag));
  ...
  const persist = useCallback(async (tag) => {
    if (isCloud) await cloudSaveTag(tag, user.id);
    else await saveTag(tag);
  }, [isCloud, user?.id]);
  ...
  const removeTag = useCallback(async (id) => {
    if (isCloud) await cloudDeleteTag(id);
    else await dbDeleteTag(id);
